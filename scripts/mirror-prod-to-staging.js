...
import { parseFlag, hasFlag, initApp, guardWriteTarget, wipeCollections, resolveClaimUid, PROD_PROJECT_ID, STAGING_PROJECT_ID, } from './lib/admin.js';
...
async function readSourceUsers() {
...
async function main() {
    ...
    const claimSource = claimUid
        ? users.reduce((a, b) => (b.readings.length > a.readings.length ? b : a), users[0])
        : null;
    if (claimSource) {
        console.log(`Re-keying source user ${claimSource.uid} (${claimSource.readings.length} readings) -> ${claimUid}`);
    }
    if (WIPE && !DRY_RUN) {
...
//# sourceMappingURL=mirror-prod-to-staging.js.map