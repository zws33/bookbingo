 ...
 */
export function initApp(projectId, appName) {
    const app = appName
 ...
 * through here.
 */
export function guardWriteTarget(projectId) {
    if (projectId === PROD_PROJECT_ID) {
        console.error(`Refusing to run: ${projectId} is PRODUCTION.\n` +
            'These scripts never write to prod. Target staging instead:\n' +
            `  --project ${STAGING_PROJECT_ID}`);
        process.exit(1);
    }
    if (process.env.FIRESTORE_EMULATOR_HOST) {
        console.warn(`Note: FIRESTORE_EMULATOR_HOST=${process.env.FIRESTORE_EMULATOR_HOST} is set ` +
 ...
 */
export async function wipeCollections(db, names) {
 ...
 */
export async function resolveClaimUid(auth, opts) {
    if (opts.uid)
        return opts.uid;
    if (!opts.email)
        return null;
    try {
        ...
        console.error(`Could not resolve a uid for "${opts.email}" in this project.\n` +
