...
if (projectFlagIndex === -1 || !args[projectFlagIndex + 1]) {
    ...
    console.error('  Example: tsx scripts/migrate-book-identity.ts --project bookbingo-staging --dry-run');
    process.exit(1);
}
 ...
 */
async function repointCollectionGroup(db, group, remap) {
...
async function runRekey() {
    ...
    const remap = new Map(); // oldId -> newId (only where they differ)
    let collapsed = 0;
    ...
    for (const [newId, docs] of groups) {
        ...
        if (olKey) {
            const enrichedAt = olDoc?.externalIds?.openLibrary?.enrichedAt ??
                olDoc?.createdAt ??
        ...
        if (docs.length > 1) {
            ...
            console.log(`  Collapsing ${docs.length} docs -> ${newId} ("${newDoc.title}" by ${newDoc.author})`);
        }
        if (DRY_RUN) {
            for (const d of docs) {
                if (d.id !== newId)
                    console.log(`    [DRY-RUN] ${d.id} -> ${newId}`);
            }
        }
        else {
...
//# sourceMappingURL=migrate-book-identity.js.map