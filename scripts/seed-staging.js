...
import { parseFlag, hasFlag, initApp, guardWriteTarget, wipeCollections, resolveClaimUid, STAGING_PROJECT_ID, } from './lib/admin.js';
import { BOOKS, USERS, bookIdFor } from './lib/dataset.js';
...
const booksByHandle = new Map(BOOKS.map((b) => [b.handle, b]));
function daysAgo(n) {
    const d = new Date();
...
function densestUser() {
    return USERS.reduce((a, b) => b.readings.length > a.readings.length ? b : a);
}
...
async function main() {
    ...
    if (claimUid) {
        console.log(`Re-keying persona "${claimed.name}" (${claimed.readings.length} readings) -> ${claimUid}`);
    }
    /** The uid this persona's data is actually written under. */
    const effectiveUid = (u) => claimed && u.uid === claimed.uid ? claimUid : u.uid;
    if (WIPE && !DRY_RUN) {
    ...
    for (const u of USERS) {
        for (const r of u.readings)
            referenced.add(r.book);
        for (const t of u.tbr)
            referenced.add(t.book);
    }
    // createdBy = effective uid of the first persona (in USERS order) to reference it.
    const creatorOf = (handle) => {
        for (const u of USERS) {
            if (u.readings.some((r) => r.book === handle) ||
    ...
    for (const handle of referenced) {
        ...
        if (!book)
            throw new Error(`Unknown book handle referenced: ${handle}`);
