...
- **Scoring logic** is in `lib/core/src/scoring.ts`. The scoring algorithm rewards volume and variety while penalizing imbalance; each term's rationale is a comment on the function that implements it. `scoring.test.ts` holds worked scenarios with expected scores — treat those as the specification.
