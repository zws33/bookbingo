...
> The authoritative record of what the system _is_ — as opposed to how it was
...
utilities (`bg-blue-600`, `text-gray-800`) throughout. Adopting the _architecture_
is worthwhile.

The reference's specific _values_ and _mechanism_ should **not** be copied verbatim:
...
_translated_ from the JS object into `@theme` CSS variables. Example:
...
editorial treatments sit on top of. The identity is applied via token _values_
...
- **One-way-ish:** token _names_ become a de-facto contract across components;
