...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Firestore path shape is written literally    | Done — `app/web/src/data/*` owns every books, readings, TBR, and users path. `lib/users.ts` (`saveUserProfile`) is the last holdout                   | Moving readings to `/challenges/{cid}/readings` means editing every one in lockstep |
