...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Firestore path shape is written literally    | `app/web/src/data/*` (done) plus `lib/tbr.ts`, which still writes `/users/{id}/readings` and `/users/{id}/tbr` directly                               | Moving readings to `/challenges/{cid}/readings` means editing every one in lockstep |
