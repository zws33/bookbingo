...
function getAltTiles(bookName, parentTile, tileCounts) {
  ...
  for (const tile of applicableNotChosen) {
    ...
    if (
      parentCount - tileCount >= 2 &&
      (tile != 'dialectics' || dialecticsPairs[bookName].inCart)
    ) {
      altTiles.push({
        name: tile,
        count: tileCount,
...
function calculateStandardDeviation(tileCounts) {
  ...
  const counts = Object.entries(tileCounts)
    .filter(([tile]) => !manualTiles.includes(tile))
...
function generateBingoFormat() {
  ...
  const sortedTiles = Object.entries(tileCounts).sort((a, b) => {
  ...
  for (const [tile, booksList] of sortedTiles) {
    ...
    for (const bookName of booksList) {
      const book = books[bookName];
      const tierLabel = book.is_unlimited
        ? 'Tier 1 - unlimited'
        : `Tier ${book.tier}`;
  ...
  const nonManualTileCount = Object.keys(tileCounts).filter(
    (t) => !manualTiles.includes(t),
  ).length;
  ...
  for (const [parentTile, booksList] of sortedTiles) {
    ...
    if (booksWithAltTiles.length > 0) {
      ...
      for (const book of booksWithAltTiles) {
        ...
        const sourceLabel =
          book.source === 'wishlist' ? ' - from wishlist' : '';
        const prefix = book.tier === 1 ? '**' : '';
        ...
        for (const altTile of book.altTiles) {
          ...
          if (altTile.name === 'dialectics' && dialecticsPairs[book.name]) {
            ...
            const cartStatus = pairInfo.inCart
              ? 'in shopping cart'
...
console.log('='.repeat(80));
console.log('BINGO REFACTORING FORMAT');
console.log('='.repeat(80));
console.log(generateBingoRefactoringFormat());

console.log('='.repeat(80));
console.log('BOOK FORMAT');
console.log('='.repeat(80));
console.log(generateBookFormat());
