...
const COLOR_VARS = COLOR_GROUPS.flatMap((g) =>
  g.roles.map((r) => `--color-${r}`),
);
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-on-surface-variant mt-1">
          Live design-system reference — colors and radii are read from the CSS
          tokens at runtime, so they always match{' '}
          <code className="text-on-surface">index.css</code>. Dev only.
        </p>
      </div>
      ...
      <Section id="colors" title="Color tokens">
        <p className="text-sm text-on-surface-variant mb-6 max-w-2xl">
          Semantic MD3-style roles. Each color pairs with its <code>on-</code>{' '}
          foreground (the accessible text/icon color to use on top of it).
          Surfaces form a warm tonal ramp used for elevation instead of shadows.
        </p>
      ...
      <Section id="typography" title="Typography">
        <p className="text-sm text-on-surface-variant mb-6 max-w-2xl">
          Display/headline roles are{' '}
          <strong className="font-display">Noto Serif</strong> (editorial
          chrome); body and labels are <strong>Inter</strong>. Family (
          <code>font-display</code> / <code>font-body</code>) is a separate axis
          from the size role (<code>text-*</code>).
        </p>
        <div className="space-y-6 bg-surface-container-lowest p-6 rounded-lg border border-outline-variant">
          {TYPE_ROLES.map(({ className, role }) => (
            <div
              key={role}
              className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4 border-b border-outline-variant pb-4 last:border-0 last:pb-0"
            >
              <div className="w-40 shrink-0 text-label-caps uppercase text-primary">
                {role}
              </div>
              <div className={`${className} text-on-surface`}>
                The curated reading experience
              </div>
            </div>
          ))}
        </div>
      ...
      <Section id="shape" title="Shape & elevation">
        <div className="mb-8">
          <RowLabel
            label="Border radius"
            note="paper-sharp; rounded-lg is the card/control default"
          />
            ...
            {RADII.map(({ className, role }) => {
              ...
              return (
                <div key={role} className="flex flex-col items-center gap-2">
                  <div
                    className={`w-16 h-16 bg-surface-container-highest border border-outline-variant ${className}`}
                  />
                  <span className="text-label-caps uppercase text-on-surface text-center leading-tight max-w-[90px]">
                    {role}
                  </span>
        ...
        <div>
          <RowLabel
            label="Elevation"
            note="tonal surfaces preferred; shadows for overlays"
          />
          <div className="flex flex-wrap gap-6">
            {['shadow-sm', 'shadow', 'shadow-lg', 'shadow-xl'].map((s) => (
              <div key={s} className="flex flex-col items-center gap-2">
                <div
                  className={`w-16 h-16 bg-surface-container-lowest rounded-lg ${s}`}
                />
      ...
      <Section id="button" title="Button">
        <RowLabel
          label="Variants"
          note="base: inline-flex items-center gap-2 px-4 py-2 rounded-lg"
        />
        ...
        <RowLabel
          ...
          note="loading auto-disables and shows a Spinner"
        />
      ...
      <Section id="badges" title="Tile badges">
        <RowLabel
          ...
          note="outlined UPPERCASE label-caps at 10px"
        />
      ...
      <Section id="spinner" title="Spinner">
        ...
        <MonoNote>
          <div>
            aria-hidden decorative · currentColor fill (text-* controls it)
          </div>
        </MonoNote>
      </Section>
      ...
      <Section id="tooltip" title="Tooltip">
        <div className="flex flex-wrap items-center gap-8">
          ...
          <Tooltip
            content="Bonus for repeat books, with diminishing returns."
            side="right"
          >
        ...
        <MonoNote>
          <div>
            Radix portal · bg-inverse-surface · delay 200ms · sideOffset 6px
          </div>
        </MonoNote>
      </Section>
      ...
      <Section id="controls" title="Controls: ToggleGroup & Accordion">
        ...
        <div>
          <RowLabel
            label="Accordion"
            note="used in Library to group books by tile"
          />
      ...
      <Section id="overlays" title="Overlays: Dialog & AlertDialog">
        ...
        <MonoNote>
          <div>
            overlay bg-black/50 (scrim) · content bg-surface-container-lowest ·
            serif titles
          </div>
        </MonoNote>
      </Section>
      ...
      <Section id="toast" title="Toast">
        <div className="flex gap-3">
          <Button
            variant="secondary"
            onClick={() => showSuccess('Book saved successfully')}
          >
            Fire success
          </Button>
          <Button
        ...
        <MonoNote>
          <div>
            success: bg-success · error: bg-error · bottom-4 right-4 · 3s
            auto-dismiss
          </div>
        </MonoNote>
      </Section>
